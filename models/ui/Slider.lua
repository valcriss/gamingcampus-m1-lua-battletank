...
Slider.new = function(name, x, y, value)
    ...
    local leftSliderButton = SliderButton.new(
            slider.name .. "_leftSliderButton",
            "assets/ui/sliderButtonLeft.png",
            slider.bounds.x,
            slider.bounds.y,
            function()
                slider.data.value = math.max(0, slider.data.value - .1)
            end
    )
    ...
    local rightSliderButton = SliderButton.new(
            slider.name .. "_rightSliderButton",
            "assets/ui/sliderButtonRight.png",
            slider.bounds.x + slider.bounds.width,
            slider.bounds.y,
            function()
                slider.data.value = math.min(1, slider.data.value + .1)
            end
    )
